...
// Stars Wars Upvote & Downvote Button
d3.select(".star-wars").text(Math.floor(Math.random() * 8) + 1);

count = 0
var number = d3.select(".star-wars").append('h1').text(count)

record = []

d3.select('#up').on('click',function(){
    count += 1
    record.push([count,'Upvote'])
    number.text(count)
})

d3.select('#down').on('click',function(){
    count -= 1
    record.push([count,'Downvote'])
    number.text(count)
})
...
// Forms
// Asking for user input to filter through a data set,
// outputting the data into a table and performing summary
// statistics
var people = blood_data

// When the Submit button is clicked the function is performed
d3.select('#submit').on('click', function(event){
    // Selects the table body
    var tbody = d3.select('.patient_data')
    // Prevents the page for reloading
    d3.event.preventDefault()
    // Acquires the user inputted text
    var bloodtype = d3.select('.form-control').node().value
    // Filters the data via user input
    var filtered = people.filter(person => person.bloodType === bloodtype.toUpperCase())
    // Adds new rows to the table
    Object.entries(filtered).forEach((value,key) => {
        var row = tbody.append('tr')
            row.append('td').text(value[1].fullName)
            row.append('td').text(value[1].age)
            row.append('td').text(value[1].bloodType)
    })
    // Creates the age dataset from the filtered data
    var age = filtered.map(row => row.age)
    // Performs Summary Stats
    var list = d3.select('.stats')
    list.append('h3').text('Age Related Stats')
    list.append('li').text(`Mean: ${math.mean(age)}`)
    list.append('li').text(`Median: ${math.median(age)}`)
    list.append('li').text(`Mode: ${math.mode(age)}`)
    list.append('li').text(`Variance: ${math.var(age)}`)
    list.append('li').text(`Standard Deviation: ${math.std(age)}`)
})
d3.select('.patient').attr('class','table table-striped')